...
function writeManifest(cwd, plugins) {
    const dir = join(cwd, ".phoenix");
    if (!existsSync(dir))
        mkdirSync(dir, { recursive: true });
    writeFileSync(join(cwd, PLUGINS_MANIFEST), JSON.stringify({ plugins }, null, 2) + "\n", "utf8");
}
export async function cmdPlugin(action, arg) {
