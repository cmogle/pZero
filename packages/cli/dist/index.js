...
program
    ...
    .action((spec) => cmdPlugin("install", spec)))
    .addCommand(new Command("list").alias("ls").description("List installed plugins").action(() => cmdPlugin("list")))
    .addCommand(new Command("remove")
    .alias("rm")
    .argument("<plugin-id>", "Plugin id to remove")
    .action((id) => cmdPlugin("remove", id)));
program.parse();
