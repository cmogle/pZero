...
export class ProcessManager {
  ...
  start(pid: ProcessId): Process | undefined {
    ...
    if (p.state !== "created" && p.state !== "blocked") return undefined;
    p.state = "ready";
    return p;
  }
  ...
  pause(pid: ProcessId): Process | undefined {
    const p = this.processes.get(pid);
    if (!p) return undefined;
    if (p.state !== "running") return undefined;
    p.state = "ready";
    return p;
  }
  ...
  resume(pid: ProcessId): Process | undefined {
    const p = this.processes.get(pid);
    if (!p) return undefined;
    if (p.state !== "ready") return undefined;
    p.state = "running";
    return p;
  }
  ...
  block(pid: ProcessId): Process | undefined {
    const p = this.processes.get(pid);
    if (!p) return undefined;
    if (p.state === "terminated") return undefined;
    p.state = "blocked";
    return p;
  }
